...
| **Survival & Hazard Engine** | Kaplan-Meier & Cox PH | Time-to-Default & Prepayment Curves | `penalizer=0.1` (Ridge regularization), C-stat: `0.6986` |
| **Anomaly Detection Layer** | Hybrid Arbitrator (Isolation Forest + Deterministic Rules + Servicer Reconciler) | Record-level Anomaly Score & Action Precedence | Weights: $w_{\text{Rule}}=49.6\%$, $w_{\text{ML}}=38.2\%$, $w_{\text{Serv}}=8.2\%$, $w_{\text{DQ}}=4.0\%$ |
...
2. **Collateral Leverage (LTV Bands)**: Low ($\le 60\%$), Moderate ($61-80\%$), High ($81-90\%$), Elevated ($91-95\%$), Super-High ($>95\%$).
...
| :--- | :--- | ---: | ---: | ---: | ---: | ---: |
| **`next_12m_default_flag`** | Scaled Logistic Regression | 0.1622 | **0.1764 – 0.3055** | **0.8568** | **0.3439** (`@0.22`) | 0.0273 |
| **`next_12m_prepayment_flag`** | Scaled Logistic Regression | 0.3791 | **0.6233** | **0.7138** | **0.5501** (`@0.26`) | 0.1943 |
| **`next_3m_delinquency_flag`** | Scaled Logistic Regression | 0.3134 | **0.3003 – 0.4169** | **0.7906** | **0.3917** (`@0.16`) | 0.0253 |
| **`next_6m_delinquency_flag`** | Scaled Logistic Regression | 0.3019 | **0.2469 – 0.3376** | **0.7306** | **0.3515** (`@0.17`) | 0.0442 |
| **`exception_required`** | Scaled Logistic Regression | 0.6079 | **0.9863** | **0.9998** | **0.9711** (`@0.26`) | 0.0013 |
| **`next_state` (Multi-class)** | Decision Tree Baseline | Macro-F1: 0.5765 | **Macro-F1: 0.6243** | **Weighted-F1: 0.9320** | — | — |
...
1. **Strict Chronological 3-Way Split**: Data is ordered globally by `reporting_month`. Split into Train (70%), Calibration (15%), and untouched Validation (15%).
2. **Transition-Only Universe Masking**:
   - Supervised models exclude terminal records (`default_flag == 1` or `prepayment_flag == 1`) from training matrices, preventing artificial memorization of pre-existing default states.
3. **Inference Lag Persistence**:
   - `FeatureEngineer.history_tail_df` stores the boundary state per loan, eliminating inference lag collapse on forward test batches.
4. **Missingness Sentinel Masking**:
   - Missing/unknown ordinals (`-1`) are masked to `NaN` before computing interaction products (`dti_x_ltv`, `distress_score`), eliminating false prime risk distortions.
...
- **Proxy Variable Mitigation**: Geographic features (`state`) are evaluated solely as macroeconomic and legal foreclosure timeline proxies. Disparate impact ratios are audited across geographic clusters.
...
*Model Card compiled in accordance with Intain AI Track 2026 Standards.*
